...
#
# Creates:
#   sorted.nq[.fmt2].meta.json (sorted dump)
#   unsorted.nq[.fmt1].meta.json, sorted.nq[.fmt2].meta.json (JSON metadata files)
#
...
# Parse arguments
if [ $# -lt 2 ]; then
    echo "Usage: nqpatch track sort unsorted{.nt|.nq}[.fmt] sorted{.nt|.nq}[.fmt2] [SORT_OPTIONS...]" >&2
    echo "" >&2
    echo "Create tracking metadata for a dump sort operation." >&2
    echo "Compression is auto-detected from output filename extension." >&2
    exit 1
fi
