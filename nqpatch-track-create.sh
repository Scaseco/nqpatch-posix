...
#
# Creates:
#   old.meta.json, new.meta.json, patch.meta.json (JSON metadata files)
#   patch.meta.json includes sha1-from (same as old.meta.json.sha1)
#   patch.meta.json includes sha1-to   (same as new.meta.json.sha1)
#
...
# Parse arguments
if [ $# -lt 2 ]; then
    echo "Usage: nqpatch track create old.nq new.nq [patch.rdfp[.gz|.bz2]]" >&2
    echo "" >&2
